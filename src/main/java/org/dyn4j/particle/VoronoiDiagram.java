...
public class VoronoiDiagram {
  ...
  public void generate(float radius) {
    ...
    for (int k = 0; k < m_generatorCount; k++) {
      ...
      if (g.center.x < lower.x) {
    	  lower.x = g.center.x;
      } else if (g.center.x > upper.x) {
    	  upper.x = g.center.x;
      }
      
      if (g.center.y < lower.y) {
    	  lower.y = g.center.y;
      } else if (g.center.y > upper.y) {
    	  upper.y = g.center.y;
      }
    }
    ...
    for (int iteration = 0; iteration < maxIteration; iteration++) {
      ...
      while (!queue.empty()) {
        ...
        Generator b = k;
        if (a != b) {
          float ax = (float) (a.center.x - x);
          float ay = (float) (a.center.y - y);
          float bx = (float) (b.center.x - x);
          float by = (float) (b.center.y - y);
