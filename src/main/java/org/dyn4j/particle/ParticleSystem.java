...
import org.dyn4j.particle.VoronoiDiagram.VoronoiDiagramCallback;
import org.dyn4j.particle.callbacks.ParticleDestructionListener;
import org.dyn4j.particle.callbacks.ParticleQueryCallback;
import org.dyn4j.particle.callbacks.ParticleRaycastCallback;
import org.dyn4j.particle.callbacks.QueryCallback;
...
public class ParticleSystem {
	...
	//      Vector2 vi = m_positionBuffer.data[i];
	//      for (int j = i + 1; j < m_count; j++) {
	//        Vector2 vj = m_positionBuffer.data[j];
	...
	public void queryAABBWorld(QueryCallback callback, AABB aabb) {
		List<DetectResult> results = new ArrayList<DetectResult>();
		m_world.detect(aabb, false, results);

		for (DetectResult result : results) {
			//result.fixture
			callback.reportFixture(result.getFixture(),result.getBody());
		}
	}

	private final AABB temp = new AABB(0);
	...
	public int destroyParticlesInShape(Shape shape, Transform xf, boolean callDestructionListener) {
		...
		temp.set(shape.createAABB(xf));  //shape.computeAABB(temp, xf, 0);
	...
	private final AABB temp2 = new AABB(0);
	...
	public ParticleGroup createParticleGroup(ParticleGroupDef groupDef) {
		...
		if (groupDef.shape != null) {
			...
			transform.setTranslation(groupDef.position);
			transform.setRotation(groupDef.angle);
			AABB aabb = temp;
			temp.set(shape.createAABB(identity));
			/*
      ...
      for (int childIndex = 0; childIndex < childCount; childIndex++) {
        if (childIndex == 0) {
          ...
          aabb.combine(childAABB);
			 ...
			 */
			final float upperBoundY = (float) aabb.getMaxY();
			final float upperBoundX = (float) aabb.getMaxX();
			for (float y = MathUtils.floor((float) (aabb.getMinY() / stride)) * stride; y < upperBoundY; y +=
					stride) {
				for (float x = MathUtils.floor((float) (aabb.getMinX() / stride)) * stride; x < upperBoundX; x +=
						stride) {
					Vector2 p = tempVec;
					p.x = x;
					p.y = y;
					if (shape.contains(p, identity)){//testPoint(identity, p)) {
						p=transform.getTransformed(p); // Transform.mulToOut(transform, p, p);
						particleDef.position.x = p.x;
						particleDef.position.y = p.y;
						p.subtract(groupDef.position);//p.subLocal(groupDef.position);
						
						//cross to out
						particleDef.velocity.x = -groupDef.angularVelocity * p.y;
						particleDef.velocity.y = groupDef.angularVelocity * p.x;
	...
	void destroyParticleGroup(ParticleGroup group) {
		...
		if (getParticleDestructionListener() != null) {
			getParticleDestructionListener().sayGoodbye(group);
		}
	...
	public void addContact(int a, int b) {
		...
		if (d2 < m_squaredDiameter) {
			if (m_contactCount >= m_contactCapacity) {
				...
				int newCapacity =
						m_contactCount != 0 ? 2 * m_contactCount : minParticleBufferCapacity;
				m_contactBuffer =
	...
	public void updateContacts(boolean exceptZombie) {
		...
		for (int p = 0; p < m_proxyCount; p++) {
			...
			proxy.tag = computeTag((float)(m_inverseDiameter * pos.x), (float)(m_inverseDiameter * pos.y));
		}
	...
	public void updateBodyContacts() {
		...
		Vector2 initial = m_positionBuffer.data[0];
		...
		AABB aabb = new AABB(
				minX - m_particleDiameter,
				minY - m_particleDiameter,
				maxX + m_particleDiameter,
				maxY + m_particleDiameter);

		m_bodyContactCount = 0;
		
		ubccallback.system = this;
		queryAABBWorld(ubccallback, aabb);
	}

	private SolveCollisionCallback sccallback = new SolveCollisionCallback();

	public void solveCollision(Step step) {
		...
		for (int i = 0; i < m_count; i++) {
			...
			final float p1x = (float) p1.x;
			final float p1y = (float) p1.y;
			final float p2x = (float) (p1x + step.getDeltaTime() * v.x);
			final float p2y = (float) (p1y + step.getDeltaTime() * v.y);
			final float bx = (p1x < p2x) ? p1x : p2x;
			final float by = (p1y < p2y) ? p1y : p2y;
			minX = (minX < bx) ? minX : bx;
			minY = (minY < by) ? minY : by;
			final float b1x = (p1x > p2x) ? p1x : p2x;
			final float b1y = (p1y > p2y) ? p1y : p2y;
			maxX = (maxX > b1x) ? maxX : b1x;
			maxY = (maxY > b1y) ? maxY : b1y;
		}
		...
		queryAABBWorld(sccallback, aabb);
	}
	...
	public void solve(Step step) {
		...
		final float gravityx = (float) (step.getDeltaTime() * m_gravityScale * m_world.getGravity().x);
		final float gravityy = (float) (step.getDeltaTime() * m_gravityScale * m_world.getGravity().y);
		float criticalVelocytySquared = getCriticalVelocitySquared(step);
		for (int i = 0; i < m_count; i++) {
			Vector2 v = m_velocityBuffer.data[i];
			v.x += gravityx;
			v.y += gravityy;
			float v2 = (float) (v.x * v.x + v.y * v.y);
		...
		for (int i = 0; i < m_count; i++) {
			...
			pos.x += step.getDeltaTime() * vel.x;
			pos.y += step.getDeltaTime() * vel.y;
		}
		updateBodyContacts();
	...
	void solvePressure(Step step) {
		...
		for (int i = 0; i < m_count; i++) {
			...
			float h =
					pressurePerWeight
					* MathUtils.max(0.0f, MathUtils.min(w, maxParticleWeight)
							- minParticleWeight);
			m_accumulationBuffer[i] = h;
		}
		// applies pressure between each particles in contact
		float velocityPerPressure = (float) (step.getDeltaTime() / (m_density * m_particleDiameter));
		for (int k = 0; k < m_bodyContactCount; k++) {
			ParticleBodyContact contact = m_bodyContactBuffer[k];
			int a = contact.index;
			Body b = contact.body;
			...
			b.applyImpulse(f, p);//b.applyLinearImpulse(f, p, true);
		}
		...
		for (int k = 0; k < m_contactCount; k++) {
			...
			final float fx = (float) (velocityPerPressure * w * h * n.x);
			final float fy = (float) (velocityPerPressure * w * h * n.y);
	...
	void solveDamping(Step step) {
		...
		for (int k = 0; k < m_bodyContactCount; k++) {
			...
			Vector2 n = contact.normal;
			Vector2 p = m_positionBuffer.data[a];
			final float tempX = (float) (p.x-b.getWorldCenter().x);//p.x - b.m_sweep.c.x;
			final float tempY = (float) (p.y-b.getWorldCenter().y);//p.y - b.m_sweep.c.y;
			final Vector2 velA = m_velocityBuffer.data[a];
			// getLinearVelocityFromWorldPointToOut, with -= velA
			float vx = (float) (-b.getAngularVelocity() * tempY + b.getLinearVelocity().x - velA.x);
			float vy = (float) (b.getAngularVelocity() * tempX + b.getLinearVelocity().y - velA.y);
			// done
			float vn = (float) (vx * n.x + vy * n.y);
			if (vn < 0) {
				final Vector2 f = tempVec;
		...
		for (int k = 0; k < m_contactCount; k++) {
			...
			final float vx = (float) (velB.x - velA.x);
			final float vy = (float) (velB.y - velA.y);
			float vn = (float) (vx * n.x + vy * n.y);
			if (vn < 0) {
				float fx = (float) (damping * w * vn * n.x);
				float fy = (float) (damping * w * vn * n.y);
				velA.x += fx;
				velA.y += fy;
				velB.x -= fx;
				velB.y -= fy;
	...
	public void solveWall(Step step) {
		for (int i = 0; i < m_count; i++) {
	...
	void solveRigid(final Step step) {
		for (ParticleGroup group = m_groupList; group != null; group = group.getNext()) {
			if ((group.m_groupFlags & ParticleGroupType.b2_rigidParticleGroup) != 0) {
				group.updateStatistics();
				Vector2 temp = tempVec;
				Vector2 cross = tempVector2;
				Rot rotation = tempRot;
				rotation.set((float) (step.getDeltaTime() * group.m_angularVelocity));
				Rot.mulToOutUnsafe(rotation, group.m_center, cross);
				temp.set(group.m_linearVelocity).product(step.getDeltaTime()).add(group.m_center).subtract(cross);
				tempXf.setTranslation(temp);///tempXf.p.set(temp);
				tempXf.setRotation(rotation.getAngle());//tempXf.q.set(rotation);
				///Transform.mulToOut(tempXf, group.m_transform, group.m_transform);
				group.m_transform.rotate(tempXf.getRotation());
				group.m_transform.translate(tempXf.getTranslation());
				final Transform velocityTransform = tempXf2;
				//velocityTransform.p.x = step.getInverseDeltaTime() * tempXf.p.x;
				//velocityTransform.p.y = step.getInverseDeltaTime() * tempXf.p.y;
				velocityTransform.setTranslation(tempXf.getTranslation().multiply(step.getInverseDeltaTime()));
				//velocityTransform.q.s = step.getInverseDeltaTime() * tempXf.q.s;
				//velocityTransform.q.c = step.getInverseDeltaTime() * (tempXf.q.c - 1);
				/*TODO*/
				for (int i = group.m_firstIndex; i < group.m_lastIndex; i++) {
					// Transform.mulToOutUnsafe(velocityTransform, m_positionBuffer.data[i], m_velocityBuffer.data[i]);
					velocityTransform.transform(m_positionBuffer.data[i]);
				}
			}
		}
	}

	void solveElastic(final Step step) {
		float elasticStrength = (float) (step.getInverseDeltaTime() * m_elasticStrength);
		for (int k = 0; k < m_triadCount; k++) {
			...
			if ((triad.flags & ParticleType.b2_elasticParticle) != 0) {
				...
				final float px = (float) (1f / 3 * (pa.x + pb.x + pc.x));
				final float py = (float) (1f / 3 * (pa.y + pb.y + pc.y));
				float rs = (float) (oa.cross(pa)+ob.cross(pb)+oc.cross(pc));//Vector2.cross(oa, pa) + Vector2.cross(ob, pb) + Vector2.cross(oc, pc);
				float rc = (float) (oa.dot(pa)+ob.dot(pb)+oc.dot(pc));//Vector2.dot(oa, pa) + Vector2.dot(ob, pb) + Vector2.dot(oc, pc);
				...
				final float roax = (float) (rc * oa.x - rs * oa.y);
				final float roay = (float) (rs * oa.x + rc * oa.y);
				final float robx = (float) (rc * ob.x - rs * ob.y);
				final float roby = (float) (rs * ob.x + rc * ob.y);
				final float rocx = (float) (rc * oc.x - rs * oc.y);
				final float rocy = (float) (rs * oc.x + rc * oc.y);
	...
	void solveSpring(final Step step) {
		float springStrength = (float) (step.getInverseDeltaTime() * m_springStrength);
		for (int k = 0; k < m_pairCount; k++) {
			...
			if ((pair.flags & ParticleType.b2_springParticle) != 0) {
				...
				final Vector2 pa = m_positionBuffer.data[a];
				final Vector2 pb = m_positionBuffer.data[b];
				final float dx = (float) (pb.x - pa.x);
				final float dy = (float) (pb.y - pa.y);
	...
	void solveTensile(final Step step) {
		...
		for (int k = 0; k < m_contactCount; k++) {
			...
			if ((contact.flags & ParticleType.b2_tensileParticle) != 0) {
				...
				final Vector2 a2B = m_accumulation2Buffer[b];
				float h = m_accumulationBuffer[a] + m_accumulationBuffer[b];
				final float sx = (float) (a2B.x - a2A.x);
				final float sy = (float) (a2B.y - a2A.y);
				float fn = (float) ((strengthA * (h - 2) + strengthB * (sx * n.x + sy * n.y)) * w);
				final float fx = (float) (fn * n.x);
				final float fy = (float) (fn * n.y);
				final Vector2 va = m_velocityBuffer.data[a];
				final Vector2 vb = m_velocityBuffer.data[b];
				va.x -= fx;
				va.y -= fy;
	...
	void solveViscous(final Step step) {
		...
		for (int k = 0; k < m_bodyContactCount; k++) {
			...
			if ((m_flagsBuffer.data[a] & ParticleType.b2_viscousParticle) != 0) {
				...
				final float tempX = (float) (p.x-b.getWorldCenter().x);//p.x - b.m_sweep.c.x;
				final float tempY =(float) (p.y-b.getWorldCenter().y);// p.y - b.m_sweep.c.y;
				final float vx = (float) (-b.getAngularVelocity() * tempY + b.getLinearVelocity().x - va.x);
				final float vy = (float) (b.getAngularVelocity() * tempX + b.getLinearVelocity().y - va.y);
				final Vector2 f = tempVec;
		...
		for (int k = 0; k < m_contactCount; k++) {
			...
			if ((contact.flags & ParticleType.b2_viscousParticle) != 0) {
				...
				final Vector2 va = m_velocityBuffer.data[a];
				final Vector2 vb = m_velocityBuffer.data[b];
				final float vx = (float) (vb.x - va.x);
				final float vy = (float) (vb.y - va.y);
	...
	void solvePowder(final Step step) {
		...
		for (int k = 0; k < m_contactCount; k++) {
			...
			if ((contact.flags & ParticleType.b2_powderParticle) != 0) {
				...
				if (w > minWeight) {
					...
					final float fx = (float) (inter * n.x);
					final float fy = (float) (inter * n.y);
					va.x -= fx;
					va.y -= fy;
					vb.x += fx;
					vb.y += fy;
	...
	void solveSolid(final Step step) {
		// applies extra repulsive force from solid particle groups
		m_depthBuffer = requestParticleBuffer(m_depthBuffer);
		float ejectionStrength = (float) (step.getInverseDeltaTime() * m_ejectionStrength);
		for (int k = 0; k < m_contactCount; k++) {
			final ParticleContact contact = m_contactBuffer[k];
			int a = contact.indexA;
			int b = contact.indexB;
	...
	void solveColorMixing(final Step step) {
	...
	void solveZombie() {
		...
		for (int i = 0; i < m_count; i++) {
			...
			if ((flags & ParticleType.b2_zombieParticle) != 0) {
				ParticleDestructionListener destructionListener = getParticleDestructionListener();
	...
	float getCriticalVelocity(final Step step) {
		return (float) (m_particleDiameter * step.getInverseDeltaTime());
	}

	float getCriticalVelocitySquared(final Step step) {
		float velocity = getCriticalVelocity(step);
		return velocity * velocity;
	}

	float getCriticalPressure(final Step step) {
	...
	public void queryAABB(ParticleQueryCallback callback, final AABB aabb) {
		...
		final float lowerBoundX = (float) aabb.getMinX();//aabb.lowerBound.x;
		final float lowerBoundY = (float) aabb.getMinY();//aabb.lowerBound.y;
		final float upperBoundX = (float) aabb.getMaxX();//aabb.upperBound.x;
		final float upperBoundY = (float) aabb.getMaxY();//aabb.upperBound.y;
		int firstProxy =
	...
	public void raycast(ParticleRaycastCallback callback, final Vector2 point1, final Vector2 point2) {
		...
		final float vx = (float) (point2.x - point1.x);
		final float vy = (float) (point2.y - point1.y);
		float v2 = vx * vx + vy * vy;
		if (v2 == 0) v2 = Float.MAX_VALUE;
		for (int proxy = firstProxy; proxy < lastProxy; ++proxy) {
			int i = m_proxyBuffer[proxy].index;
			final Vector2 posI = m_positionBuffer.data[i];
			final float px = (float) (point1.x - posI.x);
			final float py = (float) (point1.y - posI.y);
			...
			if (determinant >= 0) {
				...
				tempVec.y = py + t * vy;
				n.normalize();
				final Vector2 point = tempVector2;
	...
	static class CreateParticleGroupCallback implements VoronoiDiagramCallback {
		public void callback(int a, int b, int c) {
			...
			final float dabx = (float) (pa.x - pb.x);
			final float daby = (float) (pa.y - pb.y);
			final float dbcx = (float) (pb.x - pc.x);
			final float dbcy = (float) (pb.y - pc.y);
			final float dcax = (float) (pc.x - pa.x);
			final float dcay = (float) (pc.y - pa.y);
			...
			if (dabx * dabx + daby * daby < maxDistanceSquared
				...
				final float midPointx = (float) (1.0 / 3 * (pa.x + pb.x + pc.x));
				final float midPointy = (float) (1.0 / 3 * (pa.y + pb.y + pc.y));
				...
				triad.s = (float) (pa.cross(pb)+ pb.cross(pc)+pc.cross(pa));//   Vector2.cross(pa, pb) + Vector2.cross(pb, pc) + Vector2.cross(pc, pa);
				system.m_triadCount++;
			}
		}
	...
	static class DestroyParticlesInShapeCallback implements ParticleQueryCallback {
		...
		public boolean reportParticle(int index) {
			...
			if (shape.contains(system.m_positionBuffer.data[index], xf)){//.testPoint(xf, system.m_positionBuffer.data[index])) {
	...
	static class UpdateBodyContactsCallback implements QueryCallback {
		...
		@Override
		public boolean reportFixture(Fixture fixture,Body b) {
			if (fixture.isSensor()) {
				return true;
			}
			...
			float bm = (float) b.getMass().getMass();
			float bI = (float) (b.getMass().getInertia() - bm * b.getLocalCenter().getMagnitudeSquared());
			float invBm = bm > 0 ? 1 / bm : 0;
			float invBI = bI > 0 ? 1 / bI : 0;
			// int childCount = shape.getChildCount();
			//for (int childIndex = 0; childIndex < childCount; childIndex++) {
			AABB aabb = fixture.getShape().createAABB();//fixture.getAABB(childIndex);
			final float aabblowerBoundx = (float) (aabb.getMinX() - system.m_particleDiameter);
			final float aabblowerBoundy = (float) (aabb.getMinY() - system.m_particleDiameter);
			final float aabbupperBoundx = (float) (aabb.getMaxX() + system.m_particleDiameter);
			final float aabbupperBoundy = (float) (aabb.getMaxY() + system.m_particleDiameter);
			int firstProxy =
					lowerBound(
			...
			for (int proxy = firstProxy; proxy != lastProxy; ++proxy) {
				...
				if (aabblowerBoundx <= ap.x && ap.x <= aabbupperBoundx && aabblowerBoundy <= ap.y
					...
					float d;
					final Vector2 n = tempVec;
					//d = fixture.getShape().computeDistance(ap, childIndex, n); /* TODO*/
					Vector2 c=fixture.getShape().getCenter();
					d=(float) fixture.getShape().getFarthestPoint(new Vector2(ap.x-10*c.x,ap.y-10*c.y), b.getTransform()).distance(ap);
					if (d < system.m_particleDiameter) {
						float invAm =
								(system.m_flagsBuffer.data[a] & ParticleType.b2_wallParticle) != 0 ? 0 : system
										.getParticleInvMass();
						final float rpx = (float) (ap.x - bp.x);
						final float rpy = (float) (ap.y - bp.y);
						float rpn = (float) (rpx * n.y - rpy * n.x);
	...
	static class SolveCollisionCallback implements QueryCallback {
		...
		private final Ray input = new Ray(0);
		private final Raycast  output = new Raycast();
		private final Vector2 tempVec = new Vector2();
		private final Vector2 tempVector2 = new Vector2();

		@Override
		public boolean reportFixture(Fixture fixture,Body body) {
			if (fixture.isSensor()) {
				return true;
			}
			final Shape shape = fixture.getShape();
			...
			for (int proxy = firstProxy; proxy != lastProxy; ++proxy) {
				...
				if (aabblowerBoundx <= ap.x && ap.x <= aabbupperBoundx && aabblowerBoundy <= ap.y
					...
					body.getTransform().getTransformed(ap,temp);
					//Transform.mulToOutUnsafe(body.m_xf, temp, input.p1);
					Vector2 p1=body.getTransform().getInverseTransformed(temp);
					input.setStart(p1);
					if(av.isZero())continue;
					// input.p2.x = ap.x + step.dt * av.x;
					//input.p2.y = ap.y + step.dt * av.y;
					input.setDirection(av);
					Vector2 p2=new Vector2(ap.x + step.getDeltaTime() * av.x,ap.x + step.getDeltaTime() * av.y);
					//input.maxFraction = 1;
					double maxLength=step.getDeltaTime() * av.getMagnitude();
					//if (fixture.getShape().raycast(output, input, childIndex)) {
					if(body.getWorld().getRaycastDetector().raycast(input, maxLength, fixture.getShape(), body.getTransform(), output)){
						...
						double fraction=output.getDistance()/maxLength;
						p.x=(1 - fraction) * p1.x + fraction * p2.x
								+ linearSlop * output.getNormal().x;
						p.y =(1 - fraction) * p1.y + fraction * p2.y
								+ linearSlop * output.getNormal().y;	  
						final float vx = (float) (step.getInverseDeltaTime() * (p.x - ap.x));
						final float vy = (float) (step.getInverseDeltaTime() * (p.y - ap.y));
						av.x = vx;
						av.y = vy;
						final float particleMass = system.getParticleMass();
						final float ax = (float) (particleMass * (av.x - vx));
						final float ay = (float) (particleMass * (av.y - vy));
						Vector2 b = output.getNormal();
						final float fdn = (float) (ax * b.x + ay * b.y);
						final Vector2 f = tempVector2;
						f.x = fdn * b.x;
						f.y = fdn * b.y;
